...
describe('createElementFromConfig', () => {
  ...
  it('should create an element with nested children', () => {
    ...
    expect((el.children[0] as HTMLElement).tagName).toBe('SPAN');
    expect((el.children[0] as HTMLElement).textContent).toBe('Child1');
    expect((el.children[1] as HTMLElement).tagName).toBe('P');
    expect((el.children[1] as HTMLElement).textContent).toBe('Child2');
    expect(document.body.contains(el)).toBe(true);
  });
  ...
  it('should set value for input and select', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLInputElement).value).toBe('test');
    expect((result[0] as HTMLElement).getAttribute('value')).toBe('test');

    const selectConfig = { tag: 'select', value: 'option1' };
    const selectResult = DomUnify.createElementFromConfig(selectConfig);
    expect((selectResult[0] as HTMLSelectElement).value).toBe('option1');
    expect(selectResult[0].getAttribute('value')).toBe('option1');
    expect((selectResult[0].children[0] as HTMLInputElement).value).toBe('option1');
  });
  ...
  it('should handle non-standard keys as attributes', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLElement).getAttribute('customAttr')).toBe('value');
  });
  ...
  it('should sanitize html to prevent XSS', () => {
    ...
    expect((el.querySelector('div') as HTMLElement).textContent).toBe('Safe');
  ...
  it('should sanitize dangerous attributes (javascript:)', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect(result).toHaveLength(1);
    const a = result[0] as HTMLElement;
  ...
  it('should sanitize non-standard attributes with dangerous protocols', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLElement).getAttribute('customHref')).toBe('');
  ...
  it('should allow raw HTML if sanitize=false (with warning)', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLElement).innerHTML).toBe('<div>Unsafe</div>');
  ...
  it('should skip invalid class and id', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLElement).className).toBe('');
    expect((result[0] as HTMLElement).id).toBe('');
  ...
  it('should skip invalid dataset keys', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect(Object.keys((result[0] as HTMLElement).dataset)).toHaveLength(0);
  ...
  it('should skip invalid style keys', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect(Object.keys((result[0] as HTMLElement).style).length).toBe(0); // No valid styles
  ...
  it('should warn and skip invalid parent', () => {
    ...
    DomUnify.createElementFromConfig(config, 'invalid' as any);
  ...
  it('should handle mixed children types (string, number, boolean, null, object)', () => {
    ...
    expect((el.childNodes[0] as HTMLElement).textContent).toBe('Text');
    expect((el.childNodes[1] as HTMLElement).textContent).toBe('42');
    expect((el.childNodes[2] as HTMLElement).textContent).toBe('true');
    expect((el.childNodes[3] as HTMLElement).tagName).toBe('SPAN');
  ...
  it('should normalize single child to array', () => {
    ...
    expect((el.children[0] as HTMLElement).textContent).toBe('Single');
  });
  ...
  it('should prevent stack overflow for excessive recursion', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect(result).toHaveLength(1);
    expect((result[0] as HTMLElement).querySelectorAll('div').length).toBeLessThanOrEqual(100);
  ...
  it('should handle large number of children', () => {
    const config = { children: Array(1000).fill(null).map((_, i) => ({ tag: 'span', text: `Child${i}` })) };
    const result = DomUnify.createElementFromConfig(config);
    expect(result).toHaveLength(1);
    expect((result[0] as HTMLElement).children.length).toBe(1000);
    expect(((result[0] as HTMLElement).children[999] as HTMLElement).textContent).toBe('Child999');
  });
  ...
  it('should sanitize HTML in nested children', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    const child = (result[0] as HTMLElement).children[0];
    expect(child.innerHTML).not.toContain('<script>');
    expect((child.querySelector('div') as HTMLElement).textContent).toBe('Safe');
    expect(window.alert).not.toHaveBeenCalled();
  });
  ...
  it('should handle invalid value types', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLInputElement).value).toBe('');
    expect((result[0] as HTMLElement).getAttribute('value')).toBe('');
  });
  ...
  it('should support custom elements', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLElement).tagName).toBe('MY-CUSTOM-TAG');
    expect((result[0] as HTMLElement).textContent).toBe('Custom');
  });

  it('should support SVG elements', () => {
    const config = { tag: 'svg', children: [{ tag: 'circle', attrs: { r: '10' } }] };
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLElement).tagName).toBe('SVG');
    expect(((result[0] as HTMLElement).children[0] as HTMLElement).tagName).toBe('CIRCLE');
    expect(((result[0] as HTMLElement).children[0] as HTMLElement).getAttribute('r')).toBe('10');
  });
  ...
  it('should batch updates to avoid layout thrashing', () => {
    ...
    expect((result[0] as HTMLElement).children.length).toBe(50);
  });

  it('should handle form attributes like type', () => {
    const config = { tag: 'input', attrs: { type: 'checkbox' } };
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLInputElement).type).toBe('checkbox');
  });
  ...
  it('should prioritize text over html with warning', () => {
    ...
    const result = DomUnify.createElementFromConfig(config);
    expect((result[0] as HTMLElement).textContent).toBe('Text');
